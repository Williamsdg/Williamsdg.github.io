...
<head>
    ...
    {aerial_libs}<style>
        *,*::before,*::after{{margin:0;padding:0;box-sizing:border-box}}
        ...
        /* TOPBAR ACTIONS */
        .topbar-action{{background:var(--surface);border:1px solid var(--border);color:var(--text-soft);border-radius:8px;padding:8px 14px;font-size:.78rem;font-weight:600;letter-spacing:0.01em;cursor:pointer;font-family:inherit;display:inline-flex;align-items:center;gap:8px;transition:all .2s}}
        .topbar-action .ta-icon{{display:inline-block}}
        .topbar-action .ta-label{{display:inline-block}}
        ...
        @media(max-width:768px){{
            ...
            .main{{margin-left:0;padding-bottom:calc(86px + env(safe-area-inset-bottom))}}
            .content{{padding:16px 14px 24px;max-width:100%}}
            .topbar{{padding:12px 14px;gap:8px}}
            .topbar h1{{font-size:1.15rem}}
            .topbar-right{{gap:6px}}
            .topbar-action{{padding:8px 10px;gap:0;min-width:36px;justify-content:center}}
            .topbar-action .ta-label,.topbar-action .ta-kbd{{display:none}}
            .topbar-action .ta-icon{{font-size:1rem;line-height:1}}
            ...
            .card-header{{padding:16px 18px 14px;gap:8px}}
            ...
            .modal{{max-width:100%;max-height:88vh;border-radius:18px 18px 0 0;animation:slideUp .3s cubic-bezier(0.2,0.8,0.2,1)}}
            .modal-header{{padding:18px 18px 14px}}
            .modal-header h2{{font-size:1.1rem}}
            .modal-body{{padding:18px}}
            .modal-footer{{padding:14px 18px calc(14px + env(safe-area-inset-bottom));flex-direction:column-reverse;align-items:stretch;gap:8px;position:sticky;bottom:0;background:rgba(15,21,40,0.96);backdrop-filter:blur(12px)}}
            .modal-footer .btn{{width:100%;justify-content:center;padding:13px 16px}}
            .bulk-bar{{position:relative;top:auto;padding:12px 14px;border-radius:10px;font-size:.85rem;flex-direction:column;align-items:stretch;gap:8px}}
            .bulk-bar .btn-group{{width:100%;justify-content:flex-start;flex-wrap:wrap}}
            .bulk-bar .btn-group .btn{{flex:1 1 calc(50% - 6px);min-width:0}}
            .palette-overlay{{padding:16px 12px}}
            .palette-input{{padding:16px 18px;font-size:.95rem}}
            .palette-results{{max-height:60vh}}
            .palette-item{{padding:10px 18px}}
            .estimate-preview{{padding:18px 16px}}
            .ep-header{{flex-direction:column;gap:14px;margin-bottom:18px;padding-bottom:14px}}
            .ep-est-num{{text-align:left}}
            .ep-parties{{grid-template-columns:1fr;gap:14px;margin-bottom:18px}}
            .ep-totals .line{{gap:18px}}
            .ep-totals .line.total{{font-size:1.2rem}}
            .ep-table th,.ep-table td{{padding:10px 8px;font-size:.82rem}}
            #aerialMap,#routeMap{{height:380px;min-height:380px}}
            #aerialResult > div:last-child{{grid-template-columns:repeat(2,1fr) !important;gap:14px !important}}
            .sched-card{{padding:14px 14px 12px;border-radius:12px}}
            .sched-card-actions{{gap:6px}}
            .sched-card-actions .btn{{min-width:0;flex:1 1 calc(50% - 6px);padding:9px 8px;font-size:.74rem}}
            .annotation-tools{{padding:10px;gap:6px}}
            .ann-btn{{padding:6px 10px;font-size:.74rem}}
            .ann-btn[onclick*="undoAnn"]{{margin-left:0 !important}}
            .toast{{bottom:calc(82px + env(safe-area-inset-bottom));right:14px;left:14px;max-width:none;padding:12px 16px;font-size:.85rem;text-align:center}}
            .tour-card{{padding:24px 22px;max-width:none;width:calc(100% - 24px)}}
            .tour-card h3{{font-size:1.3rem}}
            .schedule-card{{padding:12px 14px;gap:12px}}
            .schedule-time{{min-width:56px;font-size:.88rem}}
            .acceptance-strip{{padding:12px 14px;flex-direction:column;align-items:flex-start;gap:8px}}
        }}
        @media(max-width:380px){{
            .stat-value{{font-size:1.85rem}}
            .topbar h1{{font-size:1.05rem}}
            .content{{padding:14px 12px 20px}}
            .sched-card-actions .btn{{font-size:.7rem;padding:8px 6px}}
            .badge{{padding:2px 8px;font-size:.66rem}}
        }}
...
<div class="main">
    <div class="topbar">
        ...
        <div class="topbar-right">
            <button class="topbar-action" onclick="openPalette()" title="Search (⌘K)"><span class="ta-icon">🔍</span><span class="ta-label">Search</span><kbd class="ta-kbd" style="background:var(--g100);border-radius:4px;padding:1px 5px;font-size:.7rem;margin-left:4px">⌘K</kbd></button>
            <button class="topbar-action" onclick="shareDemo()" title="Share this demo with current data"><span class="ta-icon">↗</span><span class="ta-label">Share</span></button>
            <button class="topbar-action" onclick="startTour()" title="Take the tour"><span class="ta-icon">?</span><span class="ta-label">Tour</span></button>
