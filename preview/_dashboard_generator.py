...
<head>
    ...
    {aerial_libs}<style>
        *,*::before,*::after{{margin:0;padding:0;box-sizing:border-box}}
        ...
        /* Cmd-K palette */
        .palette-overlay{{display:none;position:fixed;inset:0;z-index:400;background:rgba(0,0,0,0.5);backdrop-filter:blur(3px);align-items:flex-start;justify-content:center;padding:88px 16px 16px}}
        .palette-overlay.open{{display:flex}}
        .palette{{background:var(--surface);border-radius:14px;width:100%;max-width:600px;box-shadow:0 24px 64px rgba(0,0,0,0.32);overflow:hidden}}
        .palette-input{{width:100%;padding:18px 20px;border:none;font-size:1rem;font-family:inherit;background:var(--surface);color:var(--text);outline:none;border-bottom:1px solid var(--border-soft)}}
        .palette-results{{max-height:400px;overflow-y:auto;padding:8px 0}}
        .palette-section{{font-size:.7rem;font-weight:700;color:var(--text-mute);text-transform:uppercase;letter-spacing:1px;padding:10px 20px 4px}}
        .palette-item{{display:flex;align-items:center;gap:14px;padding:10px 20px;cursor:pointer;font-size:.92rem}}
        .palette-item:hover,.palette-item.active{{background:color-mix(in srgb, var(--primary) 12%, transparent)}}
        .palette-item .pi-icon{{width:32px;height:32px;border-radius:8px;background:var(--g100);display:flex;align-items:center;justify-content:center;font-size:.78rem;font-weight:700;color:var(--primary);flex-shrink:0}}
        ...
        .palette-item .pi-sub{{font-size:.78rem;color:var(--text-mute);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}}
        .palette-empty{{padding:30px 20px;text-align:center;color:var(--text-mute);font-size:.9rem}}
        .palette-hint{{padding:8px 16px;font-size:.72rem;color:var(--text-mute);border-top:1px solid var(--border-soft);display:flex;justify-content:space-between;flex-wrap:wrap;gap:6px;background:var(--g50)}}
        .palette-hint kbd{{background:var(--surface);border:1px solid var(--border);padding:1px 6px;border-radius:4px;font-family:inherit;font-size:.7rem;font-weight:600}}
        /* Bulk-action bar */
        .bulk-bar{{display:none;position:sticky;top:64px;z-index:35;background:var(--primary-dark);color:#fff;padding:10px 18px;border-radius:10px;margin-bottom:12px;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;box-shadow:0 4px 16px rgba(0,0,0,0.18)}}
        .bulk-bar.show{{display:flex}}
        .bulk-bar .bb-count{{font-weight:700;font-size:.9rem}}
        ...
        .bottom-tabs{{display:none;position:fixed;left:0;right:0;bottom:0;background:var(--surface);border-top:1px solid var(--border);box-shadow:0 -2px 12px rgba(0,0,0,0.06);z-index:38;padding:6px 4px calc(6px + env(safe-area-inset-bottom));justify-content:space-around}}
        .bottom-tabs button{{flex:1;background:none;border:none;color:var(--text-mute);padding:8px 4px;display:flex;flex-direction:column;align-items:center;gap:2px;font-size:.7rem;font-weight:600;cursor:pointer;font-family:inherit;border-radius:8px}}
        .bottom-tabs button svg{{width:22px;height:22px}}
        .bottom-tabs button.active{{color:var(--primary)}}
        /* Mobile schedule cards */
        .sched-card{{background:var(--surface);border:1px solid var(--border-soft);border-radius:12px;padding:14px;margin-bottom:10px;box-shadow:0 1px 2px rgba(0,0,0,0.04)}}
        .sched-card-head{{display:flex;justify-content:space-between;align-items:flex-start;gap:10px;margin-bottom:8px}}
        .sched-card-title{{font-weight:700;font-size:.95rem;color:var(--text)}}
        .sched-card-meta{{font-size:.78rem;color:var(--text-mute);margin-top:2px}}
        .sched-card-body{{font-size:.85rem;color:var(--text);line-height:1.5}}
        .sched-card-body a{{color:var(--primary);text-decoration:none}}
        .sched-card-body strong{{color:var(--text-mute);font-weight:600;font-size:.78rem;text-transform:uppercase;letter-spacing:.5px;margin-right:6px}}
        .sched-card-actions{{display:flex;gap:6px;flex-wrap:wrap;margin-top:12px}}
        .sched-card-actions .btn{{flex:1;min-width:82px;justify-content:center}}
        ...
        @media(max-width:768px){{.sidebar{{transform:translateX(-100%)}}.sidebar.open{{transform:translateX(0)}}.sidebar-close,.sidebar-toggle{{display:block}}.main{{margin-left:0;padding-bottom:72px}}.content{{padding:18px}}.topbar{{padding:12px 16px}}.stats-row,.report-grid{{grid-template-columns:1fr}}.form-row{{grid-template-columns:1fr}}.btn{{padding:8px 14px;font-size:.84rem}}.btn-sm{{padding:6px 10px;font-size:.74rem}}.bottom-tabs{{display:flex}}.desktop-only{{display:none !important}}.mobile-only{{display:block}}.modal-footer{{flex-direction:column-reverse;align-items:stretch}}.modal-footer .btn{{width:100%;justify-content:center}}.bulk-bar{{top:60px}}.palette-overlay{{padding-top:24px}}}}
        @media print{{
...
function renderEstimates(){{
    ...
    tb.innerHTML=list.length?list.map(e=>{{
        ...
        const badge=e.status==='Sent'?'badge-sent':e.status==='Paid'?'badge-completed':'badge-pending';
    ...
    if(mob){{
        mob.innerHTML=list.length?list.map(e=>{{
            const tot=e.items.reduce((s,i)=>s+i.qty*i.price,0);
            const badge=e.status==='Sent'?'badge-sent':e.status==='Paid'?'badge-completed':'badge-pending';
            return`<div class="sched-card" onclick="previewEst('${{e.num}}')" style="cursor:pointer"><div class="sched-card-head"><div><div class="sched-card-title">${{e.client}}</div><div class="sched-card-meta">#${{e.num}} · ${{e.date}}</div></div><span class="badge ${{badge}}">${{e.status}}</span></div><div class="sched-card-body">${{e.items[0]?.desc||'Service'}}<div style="font-size:1.1rem;font-weight:800;color:var(--primary-dark);margin-top:6px">${{fmtUsd(tot)}}</div></div></div>`;
